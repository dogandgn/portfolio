...
export default function Experience() {
    ...
    <div className="space-y-10">
      {list.map((exp, index) => (
        <motion.div 
          ...
          className="relative pl-8 border-l border-gunmetal"
        >
          <span className="absolute -left-[5px] top-1.5 h-2.5 w-2.5 rounded-full bg-signal"></span>
          ...
          <div className="flex flex-wrap gap-1.5">
            {exp.tech.map((t, i) => (
              <span key={i} className="px-2.5 py-1 bg-deep border border-gunmetal rounded text-[10px] text-fog font-medium uppercase tracking-widest">
                {t}
              </span>
            ))}
          </div>
  ...
  return (
    <section id="experience" className="py-24 px-6 max-w-[1344px] mx-auto">
      ...
      <div className="mb-14">
        <h3 className="text-3xl md:text-[32px] font-bold text-white leading-snug tracking-tight mb-2">Şehir Plancısı & Kurumsal CBS Geliştiricisi</h3>
        <p className="text-ash text-sm">Kasım 2021 - Temmuz 2026</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
        <div>
          <div className="flex items-center mb-10 pb-4 border-b border-gunmetal">
            ...
            <h3 className="text-xl font-bold text-white">CBS Çalışmaları</h3>
          </div>
